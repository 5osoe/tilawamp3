...
function getAudioUrl(reciterIdx, surahIdx) {
    ...
    // استخدام السيرفر المفضل من التخزين، أو الافتراضي الموثوق
    const server = localStorage.getItem(SERVER_STORAGE_KEY) || AUDIO_SERVERS[1]; // server8 as default fallback
...
async function playTrack(index, autoPlay = true, startTime = 0) {
    ...
    try {
        ...
        // البحث عن سيرفر يعمل وتحديث التخزين
        let url = await findWorkingServer(reciterPath, padded);
