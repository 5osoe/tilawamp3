...
// 2️⃣ تعديل بيانات القراء لاستخدام المسار النسبي (path)
const allRecitersSource = [
    ...
    { id: 'namh', name: 'نعمة الحسان', path: 'namh/' },
...
// 3️⃣ دالة البحث عن أفضل سيرفر (Smart Ranking)
    ...
    try {
        const checks = serversToTest.map(server => {
            ...
            return fetch(url, { method: "HEAD", signal: controller.signal })
                .then(res => res.ok ? { ok: true, server, url } : null)
                .catch(() => null);
        });
...
// تعديل دالة Smart Resume لتتوافق مع النظام الجديد
window.confirmSmartResume = async function() {
    ...
    // استخدام نظام الفحص بدلاً من الرابط المباشر
    try {
        setLoadingState(true);
        const padded = String(currentSurahIndex + 1).padStart(3, "0");
        const reciterPath = reciters[currentReciterIndex].path;
        
        const url = await findWorkingServer(reciterPath, padded);
        audio.src = url;
...
// تم تعديل هذه الدالة لتعمل بشكل متزامن من أجل واجهة المستخدم
// تستخدم السيرفر المفضل أو الأول كافتراضي للروابط الثابتة
function getAudioUrl(reciterIdx, surahIdx) {
    const reciter = reciters[reciterIdx];
    // نستخدم السيرفر المفضل إذا وجد، أو الأول في القائمة كقيمة افتراضية للعرض
    const server = localStorage.getItem(SERVER_STORAGE_KEY) || AUDIO_SERVERS[0];
    const surahPad = String(surahIdx + 1).padStart(3, '0');
    return `${server}${reciter.path}${surahPad}.mp3`;
}
...
function changeReciter(newIndex) {
    ...
    // عند تغيير القارئ، لا نقوم بالتحميل التلقائي فوراً بل ننتظر التشغيل أو نحدث الواجهة فقط
    // إذا كان هناك سورة نشطة، نحدث المعلومات فقط
    updateReciterUI(newIndex);
    renderSurahs(); // يعيد رسم القائمة بالروابط الجديدة
    updatePlayerInfo();
}

// 4️⃣ تعديل playTrack لتشمل الفحص الذكي
async function playTrack(index, autoPlay = true, startTime = 0) {
    ...
    try {
        ...
        // التحقق مما إذا كان الرابط الحالي للصوت هو نفسه المطلوب (في حال السيرفر لم يتغير)
        // ولكن بما أننا نستخدم الفحص، فمن الأفضل التأكد دائماً عند تغيير السورة
        
        let url;
        // إذا كانت نفس السورة ونفس القارئ، ربما لا نحتاج لإعادة الفحص إذا كان الرابط يعمل
        // لكن لضمان الموثوقية سنقوم بالفحص
        url = await findWorkingServer(reciterPath, padded);
        ...
        // alert("تعذر تحميل السورة حالياً. يرجى المحاولة لاحقاً.");
    }
}
...
function renderSurahs() {
    ...
    surahNames.forEach((name, index) => {
        ...
        // هنا نستخدم getAudioUrl المتزامنة للعرض فقط
...
async function downloadWithProgress(event, url, filename) {
    ...
    try {
        // تحديث الرابط في حال كان الرابط الافتراضي لا يعمل، نحاول الحصول على رابط يعمل قبل التحميل
        // لكن هذا قد يستغرق وقتاً. نستخدم الرابط الممرر (url) الذي تم إنشاؤه بواسطة getAudioUrl
